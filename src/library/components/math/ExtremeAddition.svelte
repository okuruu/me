<script lang="ts">
    ...
    let extremeAddition: number[][] = [];
    let additionAnswer: number;
    let quizAnswer: number = 0;

    let quizVisibility: boolean = true;
    let answerVisibility: boolean = true;
...
<div class="d-flex justify-content-end">
    ...
    <span class="badge badge-light me-2" aria-label="Show Quiz Extreme Addition" on:click={() => setVisibility('quiz')}>
        <img src="/icons/elements/Quiz.svg" alt="Quiz Extreme Addition" class="h-20px">
    </span>

    <!-- svelte-ignore a11y-click-events-have-key-events -->
    <!-- svelte-ignore a11y-no-static-element-interactions -->
    <span class="badge badge-light me-2" on:click={() => setVisibility('answer')}>
...
{#if answerVisibility}
    <div class="form-group my-5">
        ...
        <input type="number" bind:value={additionAnswer} on:keyup={verifyAnswer} class="form-control h-40px bg-secondary rounded fw-bolder display-3 text-end p-1 w-100" placeholder="0"/>
    </div>
{/if}
