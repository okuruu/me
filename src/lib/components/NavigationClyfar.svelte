<script lang="ts">
    ...
    const navigationItems = [
        { id: 'results', redirectTo: '/clyfar/beranda' },
        { id: 'create', redirectTo: '/clyfar/create' },
    ];
...
<div class=" fixed bottom-0 left-0 right-0 bg-white shadow-lg">
    <div class="container mx-auto btm-nav flex justify-between p-4">
        <button on:click={() => setActive('results')} class={activeItem === 'results' ? 'active' : ''}>
            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6" /></svg>
        </button>
        <button on:click={() => setActive('create')} class={activeItem === 'create' ? 'active' : ''}>
            <img src="/icons/User.svg" class="h-4 w-4" alt="create icon" />
        </button>
    </div>
</div>